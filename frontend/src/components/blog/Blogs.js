...
function Blogs() {
  ...
  const { loading, blogs, error , blogsCount, resPerPage} = useSelector((state) => state.allBlogs);
...
<section id='products' className='container mt-5'>
  {blogs.map((blog, index) => (
    <div key={blog._id} className='card p-3 rounded' style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', marginBottom: index !== blogs.length - 1 ? '20px' : '0' }}>
      <div>
        <img
