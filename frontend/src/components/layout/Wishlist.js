...
const Wishlist=() => {
  ...
  const removeFromWishlist = (productId) => {
      // Remove the item from the wishlist based on the product ID
      const updatedWishlist = savedProducts.filter((product) => product._id !== productId);
...
<Fragment>
    <div className="checkout-container">
          ...
          {savedProducts.length === 0 ? (
            <h2 className="mt-5 text-center">No products saved in the wishlist yet!</h2>
            ) : (
              <Fragment>
                  <h2 className="mt-5 text-center"> Wishlist: <b>{savedProducts.length} items</b></h2>
                                    ...
                                    <table className="table shop_table shop_table_responsive cart" cellSpacing="0">
                                        ...
                                        <tr className="cart_item">
                                            ...
                                            <td className="product-remove" data-title="Remove">
                                                <a href="#" className="remove" aria-label="Remove this item" data-product_id="30" data-product_sku="" onClick={() => removeFromWishlist(product._id)}>×</a>
                                            </td>
                                        </tr>
                                       </Fragment>
                                        ))}
